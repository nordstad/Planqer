...
const AdminRoute = ({ children }) => {
    ...
    return (
      <CatalogPage>
        <div className="card" style={{ maxWidth: '420px', margin: '80px auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--ink-2)', marginBottom: '18px' }}>Sign in to reach the admin panel.</p>
          <button type="button" className="btn-order" onClick={() => setAuthModalOpen(true)}>
            Sign in
          </button>
        </div>
  ...
  if (!user?.is_admin) {
    return (
      <CatalogPage>
        <div className="card" style={{ maxWidth: '420px', margin: '80px auto', textAlign: 'center' }}>
          <h2 className="section-title" style={{ marginBottom: '10px' }}>Admin access required</h2>
          <p style={{ color: 'var(--ink-2)' }}>
            This account doesn't have admin rights on this instance. Ask an existing admin to grant it.
          </p>
        </div>
      </CatalogPage>
    );
