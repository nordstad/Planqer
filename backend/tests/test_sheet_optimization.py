...
class TestSheetLayout:
    ...
    def test_sheet_layout_with_parts(self):
        ...
        sheet = SheetLayout(
            sheet_width=1220, sheet_height=2440, parts=[part1, part2], kerf_width=3.0
        )
    ...
    def test_can_place_part(self):
        ...
        assert sheet.can_place_part(
            new_part, 0, 0
        )  # Bottom-left corner, should not overlap
        assert sheet.can_place_part(
            new_part, 0, 403
        )  # Above existing part (200+200+1.5+1.5=403)
        assert sheet.can_place_part(new_part, 403, 200)  # Right of existing part

        # Test invalid placements
        assert not sheet.can_place_part(
            new_part, 950, 0
        )  # Would exceed sheet width (950+100 > 1000)
        assert not sheet.can_place_part(
            new_part, 0, 950
        )  # Would exceed sheet height (950+100 > 1000)
        assert not sheet.can_place_part(
            new_part, 250, 250
        )  # Would overlap existing part
...
class TestSheetOptimization:
    ...
    def test_bottom_left_fill_simple(self):
        ...
        parts = {"small_rect": {"width": 100, "height": 100, "quantity": 2}}
    ...
    def test_get_sheet_algorithm_recommendation(self):
        ...
        # Very small problem
        small_parts = {"part1": {"width": 100, "height": 100, "quantity": 3}}
        ...
        uniform_parts = {
            f"part{i}": {"width": 200, "height": 200, "quantity": 1} for i in range(30)
        }
        ...
        large_parts = {
            f"part{i}": {"width": 100, "height": 100, "quantity": 1} for i in range(100)
        }
    ...
    def test_genetic_algorithm_small_fallback(self):
        ...
        parts = {"part1": {"width": 200, "height": 200, "quantity": 2}}
