...
class TestKerfCalculations:
    ...
    def test_kerf_affects_board_count_simple(self):
        ...
        result_with_kerf = first_fit_decreasing(
            parts, board_length, saw_blade_width=20.0
        )
    ...
    def test_kerf_affects_board_count_extreme(self):
        ...
        result_extreme_kerf = first_fit_decreasing(
            parts, board_length, saw_blade_width=30.0
        )
    ...
    def test_compute_metrics_kerf(self):
        ...
        total_waste, _warnings = compute_metrics(
            cut_list, board_length, saw_blade_width
        )
    ...
    def test_optimize_cutting_function_kerf(self):
        ...
        # With no kerf, should fit in 1 board
        result_no_kerf = optimize_cutting(
            parts, board_length, 0.0, OptimizationAlgorithm.FIRST_FIT_DECREASING
        )
        assert result_no_kerf.num_boards == 1

        # With 20mm kerf: 240 + 3*20 = 300mm > 270mm, needs 2 boards
        result_with_kerf = optimize_cutting(
            parts, board_length, 20.0, OptimizationAlgorithm.FIRST_FIT_DECREASING
        )
        assert result_with_kerf.num_boards >= 2
    ...
    def test_kerf_consistency_across_algorithms(self):
        ...
        for algorithm in algorithms:
            ...
            assert result.num_boards >= 2, (
                f"{algorithm.value} should need at least 2 boards"
            )
    ...
    def test_legacy_cutting_function_kerf(self):
        ...
        # With no kerf, should fit in 1 board
        num_boards_no_kerf, _cut_list_no_kerf = min_boards_required_with_cut_list(
            parts, board_length, saw_blade_width=0.0
        )
        assert num_boards_no_kerf == 1

        # With 15mm kerf: 210 + 2*15 = 240mm > 230mm, needs 2 boards
        num_boards_with_kerf, _cut_list_with_kerf = min_boards_required_with_cut_list(
    ...
    def test_realistic_woodworking_scenario(self):
        ...
        for board in result.cut_list:
            ...
            assert used <= board_length, (
                f"Board exceeds length: {used} > {board_length}"
            )
