...
def test_cleanup_old_tasks(clean_task_manager):
    ...
    clean_task_manager.create_task(task1_id)
    clean_task_manager.update_task(
        task1_id, status=TaskStatus.COMPLETED, end_time=old_time
    )

    # Recent completed task (1 hour ago)
    recent_time = datetime.now(UTC) - timedelta(hours=1)
    clean_task_manager.create_task(task2_id)
    clean_task_manager.update_task(
        task2_id, status=TaskStatus.COMPLETED, end_time=recent_time
    )

    # Still processing task
    clean_task_manager.create_task(task3_id)
    clean_task_manager.update_task(task3_id, status=TaskStatus.PROCESSING)
...
async def test_process_optimization_async_failure(clean_task_manager):
    ...
    with patch(
        "planqer.async_processing.run_optimization",
