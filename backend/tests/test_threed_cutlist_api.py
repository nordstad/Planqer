...
def test_3d_cutlist_success(mock_process, client):
    ...
    # Make request
    files = {"file": ("test.stl", b"fake stl content", "application/octet-stream")}
    data = {"units": "mm", "round_precision": "1", "project_name": "Test Project"}
...
@pytest.mark.skip(
    reason="Rate limiter persists across tests causing inconsistent results"
)
...
def test_3d_cutlist_project_name_sanitization(client):
    ...
    with patch("planqer.api.process_uploaded_stl") as mock_process:
        ...
        assert (
            "Test" in result["project_name"] and "Project" in result["project_name"]
