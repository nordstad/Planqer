...
class TestExtremeKerfValues:
        ...
        for board in result.cut_list:
            if board:
                ...
                assert used <= board_length, (
                    f"Board overloaded: {used} > {board_length}"
                )
    ...
    def test_all_algorithms_extreme_kerf(self):
        ...
        for algorithm in algorithms:
            ...
            assert result.num_boards >= 2, (
                f"{algorithm.value} should need multiple boards"
            )
            ...
            for board in result.cut_list:
                if board:
                    used = sum(board) + max(0, len(board) - 1) * extreme_kerf
                    assert used <= board_length, (
                        f"{algorithm.value} overloaded board: {used} > {board_length}"
                    )
    ...
    def test_legacy_function_extreme_kerf(self):
        ...
        for board in cut_list:
            if board:
                used = sum(board) + max(0, len(board) - 1) * extreme_kerf
                assert used <= board_length, (
                    f"Legacy function overloaded board: {used} > {board_length}"
                )
    ...
    def test_visualization_extreme_kerf(self):
        ...
        assert (
            "WASTE" in svg_content or len(result.cut_list) == 1
        )  # Should show waste or minimal boards
