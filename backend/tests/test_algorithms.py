...
def test_calculate_board_usage():
    ...
    # Test empty board
    used_empty, remaining_empty = calculate_board_usage(
        [], board_length, saw_blade_width
    )
...
def test_best_fit_decreasing():
    ...
    assert (
        result.num_boards <= ffd_result.num_boards
        or result.total_waste <= ffd_result.total_waste
    )
...
def test_genetic_algorithm():
    ...
    result = genetic_algorithm(
        parts, board_length, saw_blade_width, population_size=20, generations=10
    )
...
def test_branch_and_bound():
    ...
    large_parts = {
        i: 1 for i in range(10, 25)
    }  # 15 parts - should fall back to genetic
...
def test_get_algorithm_recommendation():
    ...
    assert (
        get_algorithm_recommendation(small_parts)
    ...
    assert (
        get_algorithm_recommendation(medium_parts)
    ...
    assert (
        get_algorithm_recommendation(large_parts)
        == OptimizationAlgorithm.FIRST_FIT_DECREASING
    )

    # High diversity problem -> Best Fit Decreasing
    high_diversity = {
        i: 1 for i in range(10, 30)
    }  # 20 parts, high diversity (20/20 = 1.0)
    assert (
        get_algorithm_recommendation(high_diversity)
...
def test_algorithm_consistency():
    ...
    for algorithm in OptimizationAlgorithm:
        ...
        for i, board in enumerate(result.cut_list):
            used, remaining = calculate_board_usage(
                board, board_length, saw_blade_width
            )
            assert used <= board_length, (
                f"Algorithm {algorithm.value} - Board {i} overloaded: {board} -> {used} > {board_length}"
            )
            assert remaining >= 0, (
                f"Algorithm {algorithm.value} - Board {i} negative remaining space: {remaining}"
            )

        # Check total parts count
        total_placed = sum(len(board) for board in result.cut_list)
        total_expected = sum(parts.values())
        assert total_placed == total_expected, (
            f"Part count mismatch: {total_placed} != {total_expected}"
        )
...
def test_algorithm_performance_comparison():
    ...
    ffd_result = optimize_cutting(
        parts, board_length, saw_blade_width, OptimizationAlgorithm.FIRST_FIT_DECREASING
    )
    bf_result = optimize_cutting(
        parts, board_length, saw_blade_width, OptimizationAlgorithm.BEST_FIT
    )
