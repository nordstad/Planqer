...
def upgrade():
    ...
    with op.batch_alter_table("user_projects") as batch_op:
        ...
        batch_op.create_foreign_key(
            "fk_user_projects_project_group_id",
            "project_groups",
            ["project_group_id"],
            ["id"],
            ondelete="CASCADE",
    ...
    with op.batch_alter_table("user_sheet_projects") as batch_op:
        batch_op.add_column(sa.Column("project_group_id", sa.Uuid(), nullable=True))
        batch_op.create_foreign_key(
            "fk_user_sheet_projects_project_group_id",
            "project_groups",
            ["project_group_id"],
            ["id"],
            ondelete="CASCADE",
...
def downgrade():
    with op.batch_alter_table("user_sheet_projects") as batch_op:
        batch_op.drop_constraint(
            "fk_user_sheet_projects_project_group_id", type_="foreignkey"
        )
        batch_op.drop_column("project_group_id")
    with op.batch_alter_table("user_projects") as batch_op:
        batch_op.drop_constraint(
            "fk_user_projects_project_group_id", type_="foreignkey"
        )
        batch_op.drop_column("project_group_id")
