    ...
    try:
        if args.command == "backup":
            result = create_backup(
                database_url=args.database_url, output_dir=args.output_dir
            )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="planqer", description="Operate a Planqer instance."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    backup_parser = subparsers.add_parser(
        "backup", help="Create a backup archive for the local SQLite database."
    )
    ...
    restore_parser = subparsers.add_parser(
        "restore", help="Restore a backup archive over the local SQLite database."
    )
    restore_parser.add_argument(
        "archive", type=Path, help="Backup archive created by planqer backup."
    )
    restore_parser.add_argument(
        "--database-url",
        help="SQLite database URL. Defaults to DATABASE_URL, then backend/config.yaml, then ./data/planqer.db.",
