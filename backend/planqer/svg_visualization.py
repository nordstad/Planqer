...
class SVGCuttingVisualizer:
    ...
    def _assign_colors_to_parts(self, cut_list: list[list[float]]) -> dict[float, str]:
        ...
        unique_lengths = sorted(
            {round(length, 2) for board in cut_list for length in board}
        )
        return {
            length: self.colors[i % len(self.colors)]
...
'''
    ...
    def _create_board_row(
        self,
        board: list[float],
        board_length: float,
        y_pos: int,
        part_colors: dict[float, str],
        ...
        used_length = sum(board) + saw_blade_width * (
            len(board) - 1 if len(board) > 1 else 0
        )
    ...
    def _create_legend(
        self, part_colors: dict[float, str], width: int, height: int
    ) -> str:
        ...
        for idx, (length, color) in enumerate(part_colors.items()):
            ...
            items_in_current_row = min(
                items_per_row, total_items - (row * items_per_row)
            )
    ...
    def generate_svg_cut_list(
        ...
        if isinstance(board_length, (int, float)):
            ...
            if len(board_lengths) != len(cut_list):
                ...
                board_lengths = [board_lengths[0] if board_lengths else 1000] * len(
                    cut_list
                )
                is_mixed_lengths = False
        else:
        ...
        for idx, board in enumerate(cut_list):
            used = sum(board) + saw_blade_width * (
                len(board) - 1 if len(board) > 1 else 0
            )
        ...
        svg_parts.append(
            self._create_header_section(
                width, project_name, board_lengths, total_waste, is_mixed_lengths
            )
        )
