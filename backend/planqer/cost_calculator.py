...
def apply_bulk_discount(
    quantity: int, base_price: float, bulk_discount: float, min_quantity: int
) -> float:
    """
...
def calculate_material_efficiency(cut_list, board_length, saw_blade_width=3.0):
    ...
    return (
        total_material_used / total_material_available
        if total_material_available > 0
        else 0.0
    )
...
def calculate_cost_analysis(
    ...
    for board_length, quantity in boards_needed.items():
        if board_length in board_costs:
            ...
            # Apply bulk discount
            final_price = apply_bulk_discount(
                quantity, base_price, bulk_discount, min_quantity
            )
    ...
    if board_lengths_used:
        ...
        material_efficiency = (
            (total_useful / material_bought) if material_bought else 0.0
        )
        kerf_loss = (
            sum(max(len(board_cuts) - 1, 0) for board_cuts in cut_list)
            * saw_blade_width
        )
        offcut = material_bought - total_useful - kerf_loss
        waste_cost = total_cost * (offcut / material_bought) if material_bought else 0.0
    else:
        primary_board_length = (
            max(boards_needed.keys()) if boards_needed else max(board_lengths)
        )
        material_efficiency = calculate_material_efficiency(
            cut_list, primary_board_length, saw_blade_width
        )
        ...
        waste_cost = calculate_waste_cost(
            cut_list, primary_board_length, primary_price, saw_blade_width
        )

    # Calculate total useful material length
    total_useful_material = sum(sum(board_cuts) for board_cuts in cut_list)
    cost_per_useful_material = (
        total_cost / total_useful_material if total_useful_material > 0 else 0.0
    )
    ...
    return {
        ...
        "currency": currency,
        "cost_per_board_type": {
            str(k): round(v, 2) for k, v in cost_per_board_type.items()
        },
        "boards_needed_by_type": {str(k): v for k, v in boards_needed.items()},
        "waste_cost": round(waste_cost, 2),
        "material_efficiency": round(
            material_efficiency * 100, 1
