...
def calculate_board_usage(
    board: list[float], board_length: float, saw_blade_width: float
) -> tuple[float, float]:
    """
...
def first_fit_decreasing(
    parts: dict[float, int], board_length: float, saw_blade_width: float = 0.3
) -> OptimizationResult:
    """
    ...
    for part in part_list:
        ...
        for i in range(len(boards)):
            _, remaining = calculate_board_usage(
                boards[i], board_length, saw_blade_width
            )
...
def best_fit(
    ...
    for part in part_list:
        ...
        for i in range(len(boards)):
            ...
            # When adding a part to non-empty board, we need space for part + kerf
            required_space = part + (saw_blade_width if boards[i] else 0)
            if required_space <= remaining < min_remaining:
                best_board_idx = i
...
def genetic_algorithm(
    ...
    def evaluate_fitness(permutation: list[float]) -> float:
        ...
        for part in permutation:
            ...
            # Only create a new board if the part fits. If it does not fit in
            # any board, skip it (invalid solution).
            if not placed and part <= board_length:
                boards.append([part])
...
def branch_and_bound(
    ...
    def branch_and_bound_recursive(
        remaining_parts: list[float], current_boards: list[list[float]], depth: int
    ):
        ...
        for i in range(len(current_boards)):
            _, space_remaining = calculate_board_usage(
                current_boards[i], board_length, saw_blade_width
            )
        ...
        if (
            len(current_boards) + 1 < best_num_boards and part <= board_length
