...
class SheetSVGVisualizer:
    ...
    def _assign_colors_to_parts(self, sheets: list) -> dict[str, str]:
        ...
        return {
            name: self.colors[i % len(self.colors)] for i, name in enumerate(type_names)
        }
...
'''
    ...
    def _create_sheet_layout(
        ...
        for i, part in enumerate(sheet_data["parts"]):
            ...
            if (
                part_width > 30 and part_height > 20
            ):  # Only show label if part is large enough
    ...
    def generate_sheet_visualization(
        ...
        total_height = (
            header_height + (scaled_sheet_height + sheet_spacing + 75) * rows + margin
        )  # Extra space for efficiency text
...
def generate_sheet_cutting_visualization(
    optimization_result, project_name: str | None = None
) -> str:
    """
...
def generate_saved_sheet_diagram(
    optimization_result, project_name: str | None = None
) -> str:
    """
