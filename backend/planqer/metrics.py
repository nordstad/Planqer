...
BOARDS_USED = Histogram(
    "planqer_boards_used_total", "Number of boards used in optimization", ["algorithm"]
)

WASTE_PERCENTAGE = Histogram(
    "planqer_waste_percentage", "Percentage of material wasted", ["algorithm"]
)

ACTIVE_REQUESTS = Gauge("planqer_active_requests", "Number of active HTTP requests")

CACHE_HITS = Counter("planqer_cache_hits_total", "Total cache hits")

CACHE_MISSES = Counter("planqer_cache_misses_total", "Total cache misses")

WEBSOCKET_CONNECTIONS = Gauge(
    "planqer_websocket_connections_active", "Number of active WebSocket connections"
)
...
def track_request_metrics(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            REQUEST_COUNT.labels(
                method="POST", endpoint=func.__name__, status_code=500
            ).inc()
            raise
        finally:
            duration = time.time() - start_time
            REQUEST_DURATION.labels(method="POST", endpoint=func.__name__).observe(
                duration
            )
            ACTIVE_REQUESTS.dec()
