...
from fastapi import APIRouter, FastAPI, HTTPException, BackgroundTasks, WebSocket, WebSocketDisconnect, UploadFile, File, Form
...
from planqer.metrics import (
    metrics_endpoint, track_request_metrics, track_optimization_metrics,
    track_websocket_connection
)
from planqer.logging_config import (
    configure_structured_logging, get_logger, log_optimization_request,
    log_optimization_result, log_api_request, log_websocket_event, log_error
)

from planqer.services import run_optimization
from planqer.cache import get_cache_info, clear_cache
...
from planqer.routes import (
    auth_router, settings_router, projects_router, sheet_projects_router,
    project_groups_router, admin_router,
)
...
def validate_numeric_input(value: float, min_val: float = 0.1, max_val: float = 10000.0) -> float:
    """
...
def sanitize_parts_dict(parts: dict) -> dict[float, int]:
    ...
    for length, quantity in parts.items():
        ...
        if not isinstance(quantity, int) or quantity < 1 or quantity > 10000:
            raise ValueError(f"Quantity for length {length} must be an integer between 1 and 10000")
...
sheet_router = APIRouter(prefix="/sheet-optimization", tags=["Sheet Material Optimization"])
...
class planqerRequest(BaseModel):
    ...
    def validate_algorithm(cls, v):
        ...
        if v not in valid_algorithms:
            raise ValueError(f"Invalid algorithm '{v}'. Valid options: {', '.join(valid_algorithms)}")
    ...
    def validate_currency(cls, v):
        ...
        if v not in valid_currencies:
            raise ValueError(f"Invalid currency '{v}'. Valid options: {', '.join(valid_currencies)}")
        return v
...
class ThreeDCutlistResponse(BaseModel):
    ...
    boards: list[CutListItemResponse] = []  # Board components for Wood Cutting Optimizer
    sheets: list[CutListItemResponse] = []  # Sheet components for Sheet Material Optimizer
...
class StepCutlistResponse(BaseModel):
    ...
    # Separated by component type for easy optimization workflow
    boards: list[StepCutListItemResponse] = []  # Board components for Wood Cutting Optimizer
    sheets: list[StepCutListItemResponse] = []  # Sheet components for Sheet Material Optimizer
...
@cutting_router.post("", response_model=planqerResponse, summary="Generate an optimal cutting plan for boards")
...
async def create_cutting_plan(
    ...
    logger.info(f"[{request_id}] DEBUG Host headers - Host: {request.headers.get('host')}")
    logger.info(f"[{request_id}] DEBUG Host headers - X-Forwarded-Host: {request.headers.get('x-forwarded-host')}")
    logger.info(f"[{request_id}] DEBUG Host headers - X-Original-Host: {request.headers.get('x-original-host')}")
    logger.info(f"[{request_id}] DEBUG Host headers - X-Forwarded-For: {request.headers.get('x-forwarded-for')}")
    logger.info(f"[{request_id}] DEBUG Host headers - All headers: {dict(request.headers)}")
    ...
    try:
        ...
        if total_parts > 5000:
            logger.warning(f"[{request_id}] Large request detected: {total_parts} total parts")
        ...
        if planqer_request.cost_analysis and planqer_request.cost_analysis.get('enabled'):
            ...
            board_costs_frontend = planqer_request.cost_analysis.get('board_costs', {}) or planqer_request.cost_analysis.get('boardCosts', {})
            ...
            logger.info(f"[{request_id}] Frontend cost analysis enabled with {len(board_costs_dict)} board types")
        ...
        # Handle direct cost fields (backwards compatibility)
            ...
            logger.info(f"[{request_id}] Direct board costs converted: {board_costs_dict}")
        
        logger.info(f"[{request_id}] Final cost analysis settings - Enabled: {enable_cost_analysis}, Currency: {currency}, Board types: {len(board_costs_dict)}")
        logger.info(f"[{request_id}] Cost analysis debug - Original request cost_analysis: {planqer_request.cost_analysis}")
        logger.info(f"[{request_id}] Cost analysis debug - Original enable_cost_analysis: {planqer_request.enable_cost_analysis}")
        logger.info(f"[{request_id}] Cost analysis debug - Processed enable_cost_analysis: {enable_cost_analysis}")
        logger.info(f"[{request_id}] Cost analysis debug - Board costs dict: {board_costs_dict}")
        ...
        result = run_optimization(
            parts, boards, kerf, planqer_request.project_name, algorithm, logger, planqerResponse,
        ...
        total_parts = sum(planqer_request.parts.values())
        waste_percent = (result.total_waste / result.optimal_board_length) * 100 if result.optimal_board_length > 0 else 0

        track_optimization_metrics(
            algorithm.value, True, duration,
            total_parts, result.cost, waste_percent
        )

        log_optimization_result(
            logger, request_id, algorithm.value,
            duration, result.cost, result.total_waste, True
        )
        ...
        duration = time.time() - start_time
        track_optimization_metrics(algorithm.value if 'algorithm' in locals() else "unknown", False, duration, 0, 0, 0)
        ...
        log_optimization_result(
            logger, request_id, algorithm.value if 'algorithm' in locals() else "unknown",
            duration, 0, 0, False, str(e)
        )
        
        raise HTTPException(status_code=400, detail=f"Optimization failed: {str(e)}")


@cutting_router.post("/async", summary="Start async optimization with progress tracking")
...
async def create_cutting_plan_async(
    ...
    try:
        ...
        total_parts = sum(planqer_request.parts.values())
        if total_parts > 5000:
            logger.warning(f"[{request_id}] Large async request detected: {total_parts} total parts")
        ...
        raise HTTPException(status_code=400, detail=f"Failed to create async task: {str(e)}")
...
async def get_available_algorithms():
    ...
    for alg in OptimizationAlgorithm:
        ...
        if alg == OptimizationAlgorithm.FIRST_FIT_DECREASING:
            description = "Fast algorithm that places parts in first available board (O(n²))"
...
@sheet_router.post("", response_model=SheetOptimizationResponse, summary="Generate optimal cutting plan for sheet materials")
@limiter.limit("10/minute")
async def create_sheet_optimization(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[{request_id}] Generating visualization with {len(sheets_info)} sheets")

            from planqer.sheet_visualization import generate_sheet_cutting_visualization
            visualization = generate_sheet_cutting_visualization(sheet_data, sheet_request.project_name)
        except Exception as e:
            logger.error(f"[{request_id}] Failed to generate sheet visualization: {str(e)}")
            import traceback
        ...
        raise HTTPException(status_code=400, detail=f"Sheet optimization failed: {str(e)}")
...
async def get_available_sheet_algorithms():
    ...
    for alg in SheetOptimizationAlgorithm:
        ...
        if alg == SheetOptimizationAlgorithm.BOTTOM_LEFT_FILL:
            description = "Bottom-left fill algorithm for efficient 2D rectangular packing"
            recommended_for = "Small problems or when speed is priority"
        elif alg == SheetOptimizationAlgorithm.BEST_FIT_2D:
            description = "2D best fit algorithm that minimizes waste by optimizing placement"
...
@threed_router.post("", response_model=ThreeDCutlistResponse, summary="Generate cutlist from 3D STL file")
...
async def create_3d_cutlist(
    ...
    round_precision: int = Form(1, description="Decimal places for rounding dimensions"),
    ...
    try:
        ...
        logger.info(f"[{request_id}] Component types found: {[item.type.value for item in cutlist_items]}")
        ...
        boards = [
            ...
            for item in cutlist_items if item.type.value == "board"
        ]
        ...
        sheets = [
            ...
            for item in cutlist_items if item.type.value == "sheet"
        ]
        
        logger.info(f"[{request_id}] Separated into {len(boards)} boards and {len(sheets)} sheets")
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"3D cutlist processing failed: {str(e)}"
        )


@step_router.post("", response_model=StepCutlistResponse, summary="Generate cutlist from STEP CAD file")
...
async def create_step_cutlist(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"STEP cutlist processing failed: {str(e)}"
        )
